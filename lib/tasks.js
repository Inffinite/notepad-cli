...
purgeCompleteTasks = async () => {
    await initializeDb()
    try {
        db.run(`DELETE FROM tasks WHERE completed = 'true'`)
        db.close()
        console.log('[+] Purged complete tasks successfully\n')
    } catch (error) {
        console.log('[---] Failed to delete Tasks\n')
    }
...
purgeInCompleteTasks = async () => {
    await initializeDb()
    try {
        db.run(`DELETE FROM tasks WHERE completed = 'false'`)
        db.close()
        console.log('[+] Purged incomplete tasks successfully\n')
    } catch (error) {
        console.log('[---] Failed to delete Tasks\n')
    }
